...
def load_dataset(dataset='mnist1d', datapath='mnist1d', batch_size=128, \
    ...
    if dataset == 'cifar10':
        ...
        dataset = get_dataset(get_dataset_args(), path='./mnist1d_data.pkl', download=True)
        ...
        # If data_split>1, then randomly select a subset of the data
        if data_split > 1:
            indices = torch.tensor(np.arange(len(x_train)))
            data_num = len(x_train) // data_split
